    ...
    <div className="bg-white p-6" style={{ border: "3px solid #0d0d0d", boxShadow: "4px 4px 0 #0d0d0d" }}>
      ...
      <div className="space-y-3">
        {TOP_CATEGORIES.map((cat, i) => (
          <div key={cat.name} className="flex items-center gap-3 p-3" style={{ background: i === 0 ? "#0d0d0d" : "#ffffff", border: "3px solid #0d0d0d" }}>
            <span className="w-6 h-6 flex items-center justify-center text-xs font-black shrink-0" style={{ fontFamily: "var(--font-display)", background: i === 0 ? "#d32f2f" : "#0d0d0d", color: "white" }}>
              {String(i + 1).padStart(2, "0")}
            </span>
            <span className="flex-1 text-sm font-black" style={{ fontFamily: "var(--font-display)", color: i === 0 ? "white" : "#151d1e" }}>
              {cat.name}
            </span>
            <div style={{ width: "64px", height: "10px", background: i === 0 ? "rgba(255,255,255,0.15)" : "#ecf5f6", border: `3px solid ${i === 0 ? "rgba(255,255,255,0.3)" : "#dbe4e5"}` }}>
