...
function SecretCopyRow({ secret }: { secret: string }) {
  ...
  return (
    <div className="flex items-center gap-2.5 px-3 py-2.5 mt-2.5 bg-[#f2fbfc] dark:bg-zinc-800 border-2 border-[#0d0d0d] dark:border-zinc-600">
      <code className="flex-1 font-mono text-[0.72rem] sm:text-[0.78rem] font-bold text-[#0d0d0d] dark:text-zinc-200 tracking-widest break-all">
        {secret}
      </code>
      <button
        onClick={handleCopy}
    ...
    <div className="flex gap-1 mb-5">
      {labels.map((label, i) => (
        <div key={i} className="flex-1">
          ...
          />
          <p
            className={`font-black text-[0.55rem] sm:text-[0.58rem] uppercase tracking-[0.07em] m-0 ${i <= current ? "text-[#af101a]" : "text-[#aaa] dark:text-zinc-500"}`}
          >
            {label}
          </p>
        </div>
...
function Btn({
  ...
  return (
    <button
      ...
      className={[
        "flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2.5 font-black text-[0.68rem] sm:text-[0.72rem] uppercase tracking-widest transition-all whitespace-nowrap",
        flex ? "flex-1" : "",
        loading || disabled
...
export default function TwoFASection() {
  ...
  // shared card classes
  const card =
    "border-[3px] border-[#0d0d0d] dark:border-zinc-600 shadow-[5px_5px_0_#0d0d0d] dark:shadow-[5px_5px_0_#52525b] p-4 sm:p-6";
  const dangerCard =
    "border-[3px] border-[#7f0d12] shadow-[5px_5px_0_#7f0d12] p-4 sm:p-6";

  return (
    <div className="flex flex-col gap-5">
      {/* ── Status + toggle ── */}
      <div
        className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4 py-4 border-[3px] transition-all ${
          enabled
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-black text-[0.85rem] sm:text-[0.9rem] text-[#0d0d0d] dark:text-white m-0 font-display">
              Two-Factor Authentication
            </p>
            <p className="text-[0.7rem] sm:text-[0.72rem] text-[#5b403d] dark:text-zinc-400 m-0">
              {initialLoading
                ? "Checking status..."
                : enabled
        ...
        <button
          ...
          className="flex items-center gap-2.5 bg-transparent border-none p-0 shrink-0 self-start sm:self-auto"
          style={{
      ...
      {step === "setup" && setupData && (
        <div className={`bg-white dark:bg-zinc-900 ${card}`}>
          <StepBar current={0} />
          <p className="font-black text-[0.85rem] sm:text-[0.875rem] text-[#0d0d0d] dark:text-white m-0 mb-1.5 font-display">
            Scan with your authenticator app
          </p>
          <p className="text-[0.75rem] sm:text-[0.78rem] text-[#5b403d] dark:text-zinc-400 m-0 mb-4 leading-relaxed">
          ...
          <div className="flex gap-5 items-start flex-wrap">
            <div className="border-[3px] border-[#0d0d0d] dark:border-zinc-600 shadow-[4px_4px_0_#0d0d0d] dark:shadow-[4px_4px_0_#52525b] p-2.5 bg-white shrink-0 mx-auto sm:mx-0">
              <img
      ...
      {step === "confirm" && (
        <div className={`bg-white dark:bg-zinc-900 ${card}`}>
          ...
          <p className="text-[0.75rem] sm:text-[0.78rem] text-[#5b403d] dark:text-zinc-400 m-0 mb-3.5">
      ...
      {step === "disable_confirm" && (
        <div className={`bg-red-50 dark:bg-red-950/20 ${dangerCard}`}>
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <p className="font-black text-[0.8rem] sm:text-[0.875rem] text-[#7f0d12] dark:text-red-400 m-0 font-display">
          ...
          <div className="flex gap-1.5 mb-4">
            {(["otp", "recovery"] as DisableMode[]).map((m) => (
              <button
                ...
                className={`flex-1 py-2 font-bold text-[0.68rem] sm:text-[0.75rem] uppercase tracking-[0.06em] sm:tracking-[0.08em] border-2 border-[#7f0d12] cursor-pointer transition-colors ${disableMode === m ? "bg-[#7f0d12] text-white" : "bg-white dark:bg-zinc-800 text-[#7f0d12] dark:text-red-400"}`}
              >
                {m === "otp" ? (
                  <div className="py-1 sm:py-2 flex justify-center items-center gap-1.5 sm:gap-3">
                  ...
                  <div className="py-1 sm:py-2 flex justify-center items-center gap-1.5 sm:gap-3">
                    ...
                    <span className="whitespace-nowrap">Recovery Code</span>
                  </div>
                )}
              </button>
            ))}
          </div>
          <p className="text-[0.75rem] sm:text-[0.78rem] text-[#5b403d] dark:text-zinc-400 m-0 mb-3.5 leading-relaxed">
            {disableMode === "otp"
          ...
          {disableMode === "otp" ? (
            ...
            <input
              ...
              autoFocus
              className="w-full py-3 sm:py-3.5 px-4 font-mono font-bold text-sm sm:text-base tracking-widest text-center text-[#0d0d0d] dark:text-white bg-white dark:bg-zinc-800 border-[3px] border-[#7f0d12] outline-none box-border"
              onFocus={(e) =>
      ...
      {step === "idle" && !initialLoading && (
        <div className="bg-[#e7f0f1] dark:bg-zinc-800/50 border-2 border-[#0d0d0d] dark:border-zinc-600 p-4">
          ...
          {[
            ...
            <p
              key={tip}
              className="text-[0.72rem] sm:text-[0.75rem] text-[#5b403d] dark:text-zinc-400 m-0 mb-1 pl-2.5 border-l-[3px] border-[#af101a] leading-relaxed"
            >
              {tip}
            </p>
          ))}
      ...
      {showRecovery && (
        <div className={`bg-red-50 dark:bg-zinc-900 ${card}`}>
          <p className="font-black text-[0.85rem] sm:text-[0.9rem] text-[#0d0d0d] dark:text-white m-0 mb-1.5 font-display">
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 gap-2 mb-4">
            {recoveryCodes.map((code) => (
              <code
                key={code}
                className="py-2 px-3 bg-[#f2fbfc] dark:bg-zinc-800 border-2 border-[#0d0d0d] dark:border-zinc-600 font-mono text-[0.8rem] sm:text-[0.85rem] font-bold tracking-[0.05em] text-center text-[#0d0d0d] dark:text-zinc-200"
              >
                {code}
              </code>
            ))}
