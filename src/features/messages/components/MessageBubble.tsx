import { CheckCheck, Check, FileIcon } from "lucide-react";
...
export default function MessageBubble({ msg, isLast, userId, onImageClick }: Props) {
      ...
      <div className="flex flex-col gap-0.5" style={{ maxWidth: "65%" }}>
        ...
        >
          {msg.type === "IMAGE" && msg.fileUrl ? (
            <div>
              <button onClick={() => onImageClick(msg.fileUrl!)} className="block cursor-pointer w-full">
                <img
                  src={msg.fileUrl}
                  alt={msg.fileName ?? "image"}
                  className="max-w-65 max-h-85 object-cover block hover:opacity-90 transition-opacity"
                />
              </button>
              {msg.content && <p className="px-4 py-2 text-sm">{msg.content}</p>}
            </div>
          ) : msg.type === "FILE" && msg.fileUrl ? (
            <button
              onClick={() => downloadFile(msg.fileUrl!, msg.fileName ?? "file")}
              className="flex items-center gap-3 px-4 py-3 hover:opacity-80 transition-opacity w-full text-left cursor-pointer"
            >
              <div className="w-9 h-9 flex items-center justify-center bg-[#d32f2f] shrink-0">
                <FileIcon size={16} className="text-white" />
              </div>
              <div className="min-w-0">
                <p className="text-xs font-bold truncate max-w-40">{msg.fileName ?? "File"}</p>
                {msg.fileSize != null && (
                  <p className="text-xs opacity-60">{(msg.fileSize / 1024).toFixed(1)} KB</p>
                )}
              </div>
            </button>
          ) : (
            <p className="px-4 py-2.5" style={{ lineHeight: 1.5 }}>{msg.content}</p>
          )}
        </div>

        <div className={`flex items-center gap-1 ${isMine ? "justify-end" : "justify-start"}`}>
          <span className="text-xs text-[#aaa] dark:text-zinc-400" style={{ fontSize: "10px" }}>
          ...
          {isLastMine && (
            msg.isRead ? (
              ...
              <span className="flex items-center gap-0.5 text-xs text-[#aaa] dark:text-zinc-400" style={{ fontSize: "10px" }}>
                <Check size={11} /> Sent
              </span>
            )
          )}
